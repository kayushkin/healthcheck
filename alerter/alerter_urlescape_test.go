...
func newBridgeStubRecording(t *testing.T, sessionID string, requestURI *string, hit *bool) *httptest.Server {
	...
	mux.HandleFunc("/sessions", func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(map[string]string{"session_id": sessionID, "display_name": "stub"})
	})
...
func TestCreateEscalationSessionSendsToTheSessionTheBridgeMinted(t *testing.T) {
	for _, id := range urlProbes {
		t.Run(id, func(t *testing.T) {
			...
			a.createEscalationSession("disk-root", checker.ResourceState{
				Name: "disk-root", Type: "disk", UsagePct: 97.5, Threshold: 90, Detail: "1GB free",
			})
